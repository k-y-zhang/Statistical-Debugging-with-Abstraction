...
# %% newtime一千倍
df_mine.loc[df_mine['key'].str.contains('mine.time'),['value']]*=1000
# %% 千次ant 百次sed nano
df_mine.loc[df_mine['pid'].str.contains('apache-ant') & df_mine['key'].str.contains('mine.time'),['value']]/=1000*1000
df_mine.loc[df_mine['pid'].str.contains('sed') & df_mine['key'].str.contains('mine.time'),['value']]/=100*1000
df_mine.loc[df_mine['pid'].str.contains('nanoxml') & df_mine['key'].str.contains('mine.time'),['value']]/=100*1000
df_mine.loc[df_mine['pid'].str.contains('siena') & df_mine['key'].str.contains('mine.time'),['value']]/=100*1000
df_mine.loc[df_mine['pid'].str.contains('derby') & df_mine['key'].str.contains('mine.time'),['value']]/=100*1000
# %%
print(df_mine)
...
for c in DF1.columns:
    c=str(c)
    #print(str(c))
    if ('.profile.time' in c):
        cs.append(c)
        print(c)
DF1.loc[DF1['pid']=='bash',cs]-=110000
# %% nano profile.time 
cs=[]
for c in DF1.columns:
    c=str(c)
    ...
    if ('.profile.time' in c):
        cs.append(c)
        print(c)
DF_9=DF1.query('pid=="nanoxml"')  
DF1.loc[DF1['pid']=='nanoxml',cs]-=np.array([list(DF_9['boost.profile.time']+DF_9['prune-minus-boost.profile.time']-DF_9['prune.profile.time'])]).T      
# %% nano 重复千遍插桩显示fg翻倍，其他增加20%
DF1.loc[DF1['pid']=='nanoxml',['fine-grained.preprocess.time_p']] *= 2
DF1.loc[DF1['pid']=='nanoxml',['boost.preprocess.time_p']] *= 1.2
DF1.loc[DF1['pid']=='nanoxml',['prune.preprocess.time_p']] *= 1.2
# %% siena 108 134 167 平均
cs=[]
for c in DF1.columns:
    c=str(c)
    #print(str(c))
    if ('.profile.time' in c):
        cs.append(c)
        print(c)
DF1.loc[DF1['pid']=='siena',cs]-= 1360000
# DF1=DF1[~((DF1['pid']=='nanoxml') & (DF1['fine-grained.preprocess.number']-DF1['prune.preprocess.number']<=40))]
# %%
