...
df1: pd.DataFrame = pd.read_csv('exp2_1/data.csv')
#print(df1)
df1['subject']=df1['subject'].map(lambda x: 'ant' if x=='apache-ant' else x.lower())
df1 = df1.groupby(['subject']).mean()
#print(df1)


#change the sequnce of df1
df1= df1.reindex(names)
...
g = df1[['top 1','top 5','top 10']].plot.bar(width=0.8, linewidth=2)
plt.xlabel('subject')
plt.ylabel('')
plt.ylim(0, 100)
...
dfff(g, map(lambda x: f'{x:.1f}', df1[['top1-num','top5-num','top10-num']].to_numpy().flatten()), 1.2)
