...
df1: pd.DataFrame = pd.read_csv('原实验主体/ex2rq1.csv')
print(df1)
df1 = df1.groupby(['name']).mean()
...
def dfsdf():
    ...
    for x in ["return", "scalar", "branch"]:
        df: pd.DataFrame = pd.read_csv(f'原实验主体/ex2rq2_{x}.csv')
        df = df.groupby(['name']).mean()
        print(f"## {x}", file=out)
        print(df.to_markdown(floatfmt='.2f'), file=out)
        print("", file=out)
