...
DF0 = []
ERR = ''
for x in glob.glob('data4/*.csv'): #+glob.glob('exp1_2/*.csv'):
    # print(x)
    try:
...
# %%
# %%
# DF_CC = pd.read_csv('closure_diff.csv')
# H1 = {}
...
# print(DF1['percent1'])
#sns.catplot(kind='bar', data=DF2, col='pid', x='bid',                hue='top-k', y='percent1', ci=None, edgecolor='black', linewidth=2)
plt.style.use('ggplot')  # ,hatch=['//','','+','o']
#matplotlib.rcParams['patch.facecolor'] = 'red'
#matplotlib.rcParams['axes.facecolor'] = 'red'
#matplotlib.rcParams['patch.color']= plt.cycler(hatch=['//','','+','o'])
#matplotlib.rcParams['axes.prop_cycle'] = plt.cycler(color=['0.7', '0.5', '0.0', '1.0'])
# plt.rc('patch',linewidth=3)
# plt.rc('axes',linewidth='black')

# plt.style.use('ggplot')
# sns.set_theme()
##matplotlib.rcParams['figure.facecolor'] = 'white'
#plt.rc('axes',color='white')
...
df_x['same']=df_x['max']==df_x['min']
df_x
df_x[df_x['same']==False]
#df_x

# %%
DF1=DF1.join(df_x[df_x['same']==False],on=['pid','bid'],how='inner')
#  %%
